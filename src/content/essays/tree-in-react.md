...
여기서 `id`가 `4`인 노드의 자식 노드들이 보이지 않도록 `isCollapesd` 값을 수정한다면, id가 3인 노드 그리고 id가 1인 노드의 참조를 변경해야 한다는 점이다.
...
하위 반면 배열 구조는 개별 아이템만 변경하면 되므로 불변성을 유지하는 것이 간편하다. 해당 노드 그리고 목록 전체의 참조만 신경써주면 노드 상태가 반영될 여지가 많다.
