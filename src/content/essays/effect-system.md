...
그렇게 코드들은 한 번 작성되면, 별 일 없는 이상 얌전히 놔둬지고 방치됩니다. 그러다가 코드가 점점 달라붙고 복잡해지기 시작하면 ‘기술 부채’라벨을 붙여주기도 하고요, 언젠가 리팩토링이라는 걸로 해결해야 한다고도 말합니다. 지금도 복잡해서 쩔쩔 매는 코드를 나중에 어떻게 개선한다는 건지는 저도 잘 모르겠습니다. 아무튼 리팩토링이라는 걸로 개선할 수 있다고 하네요.
